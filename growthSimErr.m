% Compare growth rates and reproduction numbers with misspecification
...
%% Add wrong serial interval

% Mismatched total infectiousness and serial interval
Lday1 = zeros(size(Lday));
distvals1 = distvals; distvals1.type = 2;
distvals1.omega = 0.333*distvals.omega;

% Serial distribution over all tday
serial1 = serialDistrTypes(nday, distvals1);
% Single omega controls distribution
PomegaErr = serial1(1/distvals1.omega);

% Iteratively generate renewal epidemic
for i = 2:nday
    % Relevant part of serial distribution
    Pomegat = PomegaErr(1:i-1);
    % Total infectiousness
    Lday1(i) = sum(Iday(i-1:-1:1).*Pomegat);    
end

% True serial interval distribution
...
% EpiFilter estimate and prediction with true and wrong data
[Re, Ip, ~, re] = allFilSmoothGrow(Rgrid, m, eta, nday, p0, Lday1, Iday, distvals1);
[Re0, Ip0, ~, re0] = allFilSmoothGrow(Rgrid, m, eta, nday, p0, Lday, Iday, distvals);

% Other estimates by smoothing incidence
Iemp = smoothdata(Iday, 'sgolay', 50, 'Degree', 3); 
%Iemp(Iemp <= 0) = 0.001; % if any non-positive values
...
% Plots of serial intervals and Lam
figure;
subplot(2, 1, 1);
plot(tday, Pomega, 'k', tday, PomegaErr, 'r', 'LineWidth', 2);
grid off; box off; xlim([tday(2) tday(end)]);
ylabel('$w_{t}$', 'FontSize', 20);
subplot(2, 1, 2);
plot(tday, Lday, 'k', tday, Lday1, 'r', 'LineWidth', 2);
grid off; box off; xlim([tday(2) tday(end)]);
ylabel('$\Lambda_{t}$', 'FontSize', 20);
...
%stairs(tday(2:end)', Iday(2:end), 'k', 'LineWidth', 2);
hold on;
...
subplot(2, 2, 4);
%stairs(tday(2:end)', Iday(2:end), 'Color', grey1, 'LineWidth', 2);
scatter(tday(2:end)', Iday(2:end), 'Marker', 'o', 'SizeData', 40, 'MarkerFaceColor',...
    grey1, 'MarkerFaceAlpha', 0.9, 'MarkerEdgeAlpha', 0.6);
hold on;
...
%% New figure comparing misspecification
...
plot(tday, Rtrue, '-', 'color', 'k', 'linewidth', 2);
hold on;
plotCIRaw(tday', Re0.mean(:, 2), Re0.low(:, 2), Re0.high(:, 2), 'b');
h = plotCIRaw(tday', Re.mean(:, 2), Re.low(:, 2), Re.high(:, 2), 'r');
plot(tday(2:end), ones(1, nday-1), '--', 'Color', 'k', 'LineWidth', 2);
grid off; box off; hold off; xlim([tday(2) tday(end)]);
ylabel('reproduction number', 'FontSize', 20);
...
if saveFig
    cd(saveFol);
    saveas(gcf, ['Fig2' namstr 'grow'], 'fig');
    cd(thisDir);
end
...
scatter(tday(2:end)', Iday(2:end), 'Marker', 'o', 'SizeData', 40, 'MarkerFaceColor',...
    grey1, 'MarkerFaceAlpha', 0.9, 'MarkerEdgeAlpha', 0.6);
hold on;
plotCIRaw(tday(2:end)', Ip0.mean(:, 2), Ip0.low(:, 2), Ip0.high(:, 2), 'b');
grid off; box off; hold off; xlim([tday(2) tday(end)]);
ylabel('case incidence', 'FontSize', 20);
...
%% Simple figure comparing misspecification
...
%leg = legend('$R_t$', '', '$\hat{R}_t$', 'Location', 'Best');
%set(leg,'Box','off')
title('A'); xlabel('time, $t$', 'FontSize', 20);
